...
import { z } from "zod"
import { expiredOffersHeader, slackOfferMessage } from "../../../utils/slack/messages"
...
export const offerRouter = t.router({
  ...
  updateExpiration: authedProcedure
    .input(
      z.object({
        offerId: z.string().cuid(),
        expiration: z.date(),
  ...
  updateZendeskTicket: authedProcedure
    .input(
      z.object({
        offerId: z.string().cuid(),
        zendeskTicket: z.string().url(),
  ...
  getByClimberAndClass: authedProcedure
    .input(
      z.object({
        climberId: z.string().cuid(),
        classId: z.string().cuid(),
  ...
  checkOffers: t.procedure
    .mutation(async ({ ctx }) => {
      try {
        ...
        const messages = offers
          .filter((offer) => {
            return offer.expiration < now
          })
          .map((offer) => {
