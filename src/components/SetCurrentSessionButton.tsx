...
const SetCurrentSessionButton = ({ selected, onRequestClose }: SetCurrentSessionButtonProps) => {
  const ctx = trpc.useContext()
  const [isOpen, setIsOpen] = useState(false);
  const toggleIsOpen = () => setIsOpen(!isOpen);

  const setCurrent = trpc.climbingSession.setCurrent.useMutation({
    onMutate: async () => {
    ...
    onSuccess: () => {
      toast.dismiss()
      toast.success(`${selected.name} ${selected.year} is now the current session`)
      toggleIsOpen()
      onRequestClose()
    }
  })
  ...
  return (
    <>
      <button
        className='p-2 rounded-lg bg-green-800 hover:scale-95 transition duration-150 ease-in shadow-md shadow-black'
