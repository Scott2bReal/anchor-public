...
const ClassAddButton = ({ toggleFunction }: ClassAddButtonProps) => {
  ...
  return (
    <>
      ...
      <span className={`${hovered ? 'translate-x-[40px] opacity-100' : 'opacity-0'} w-max absolute left-[80px] text-neutral-400 transition duration-300 ease-in-out hover:cursor-default`}>Add a class to the schedule</span>
    </>
  )
}
